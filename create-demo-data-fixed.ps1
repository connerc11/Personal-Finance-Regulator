...
try {
    Invoke-RestMethod -Uri "$API_BASE/api/transactions/user/999" -Method GET | Out-Null
...
foreach ($transaction in $sampleTransactions) {
    $result = New-Transaction -Transaction $transaction
    if ($result) {
...
Write-Host "`nSample data creation completed!" -ForegroundColor Green
Write-Host "Created $($createdTransactions.Count) transactions for demo user (ID 999)" -ForegroundColor Green
Write-Host "Demo user now has data for testing budgets and analytics." -ForegroundColor Yellow
